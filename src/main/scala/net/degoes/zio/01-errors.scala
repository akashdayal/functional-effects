...
object ErrorRecoveryOrElse extends App {
   ...
   */
  def run(args: List[String]): ZIO[ZEnv, Nothing, ExitCode] =
    failed.orElse(putStrLn("Succeeded")).exitCode
}
...
object ErrorRecoveryFold extends App {
   ...
   */
  def run(args: List[String]): ZIO[ZEnv, Nothing, ExitCode] =
    failed.fold(_ => ExitCode.success, _ => ExitCode.success)
}
...
object ErrorRecoveryCatchAll extends App {
   ...
   */
  def run(args: List[String]): ZIO[ZEnv, Nothing, ExitCode] =
    failed.catchAll(putStrLn(_)).exitCode
}
...
object ErrorRecoveryFoldM extends App {
   ...
   */
  def run(args: List[String]): ZIO[ZEnv, Nothing, ExitCode] =
    failed.foldM(putStrLn(_), putStrLn(_)).exitCode
}
...
object ErrorRecoveryEither extends App {
   ...
   */
  def run(args: List[String]): ZIO[ZEnv, Nothing, ExitCode] =
    failed.either.map(_ => ExitCode.success).exitCode
}
...
object ErrorRecoveryIgnore extends App {
   ...
   */
  def run(args: List[String]): ZIO[ZEnv, Nothing, ExitCode] =
    failed.ignore.map(_ => ExitCode.success).exitCode
}
...
object AlarmApp extends App {
  ...
  lazy val getAlarmDuration: ZIO[Console, IOException, Duration] = {
    def parseDuration(input: String): IO[NumberFormatException, Duration] =
      ZIO(Duration(input.toLong, TimeUnit.SECONDS))
        .refineToOrDie[NumberFormatException]

    def fallback(input: String): ZIO[Console, IOException, Duration] =
      ZIO(Duration(1, TimeUnit.SECONDS)).refineToOrDie[IOException]
  ...
  def run(args: List[String]): ZIO[ZEnv, Nothing, ExitCode] =
    (for {
      ...
      _   <- putStrLn(s"Sleeping for ${dur.getSeconds}")
      _   <- ZIO.sleep(dur)
    } yield ()).exitCode
}
...
object Sandbox extends App {
   ...
   */
  def run(args: List[String]): ZIO[ZEnv, Nothing, ExitCode] =
    composed.sandbox.catchAll(cause => putStrLn(cause.prettyPrint)).exitCode
}
